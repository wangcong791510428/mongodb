...
primary (ReplConn) -> try
	...
	of Conn -> {ok, Conn}  %%Conn = {connection,{"solo",27017},<0.366.0>,false,infinity}
...
fetch_member_info (ReplConn = {rs_connection, _ReplName, VConns, _, _}) ->
	%%VConns = {rs_connection,<<"OctopusDBset">>,<0.362.0>,infinity, false}}
	OldHosts_ = dict:fetch_keys (mvar:read (VConns)),
	%%OldHosts_ = [{"solo",27017}]
	{Conn, Info} = until_success (OldHosts_, fun (Host) -> connect_member (ReplConn, Host) end),
	%%Conn = {connection,{"solo",27017},<0.366.0>,false,infinity}
...
connect_member ({rs_connection, ReplName, VConns, TimeoutMS, SslOptions}, Host) ->
	...
	%%Conn = {connection,{"solo",27017},<0.366.0>,false,infinity}
	?DEBUG("~p:connect_member ~p Conn = ~p ~n", [?MODULE, ?LINE, Conn]),
...
get_connection (VConns, Host, TimeoutMS, SslOptions) -> mvar:modify (VConns,
	fun (Dict) ->
