...
do (WriteMode, ReadMode, Connection, Database, Action) -> 
	...
	{ok, Conn} ->
		%%Conn = {connection,{"solo",27017},<0.369.0>,false,infinity}
		PrevContext = get (mongo_action_context),
		?DEBUG("~p: ~p ,Conn = ~p~n", [?MODULE, ?LINE, Conn]),
...
% Get mongodb version %
get_version() ->
	Nonce = command ({buildinfo, 1}),
	?DEBUG("~p:get_version ~p Nonce = ~p", [?MODULE, ?LINE, Nonce]),
	Version = bson:at(version, Nonce),
...
auth(Username, Password) ->
	Version = get_version(),
	auth(Username, Password, Version).

%new authorisation
-spec auth(binary() , binary() ,float()) -> boolean().
auth(Username, Password, Version) when Version > 2.7 ->
	mongo_auth:auth_first_step (Username, Password);

auth(Username, Password, _Version)  ->
	mongo_auth:mongodb_old_auth (Username, Password).
