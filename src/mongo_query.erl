...
-export ([write/3, write/2, find_one/2, find/2, command/3, query_reply/1]).
...
find_one (DbConn, Query) when is_record(Query, query) ->
...
find_one (DbConn, Query) ->
	Query1 = Query #'op_msg' {batchsize = -1},
	Reply = mongo_connect:call (DbConn, [], Query1),
	{0, Docs} = query_reply (Reply),  % batchsize negative so cursor should be closed (0)
	case Docs of [] -> {}; [Doc | _] -> {Doc} end.
...
find (DbConn, Query) when is_record(Query, query) ->
	Reply = mongo_connect:call (DbConn, [], Query),
	mongo_cursor:cursor (DbConn, Query #'query'.collection,
		Query #'query'.batchsize, Query #'op_msg'.slaveok, query_reply (Reply));

find (DbConn, Query) ->
	Reply = mongo_connect:call (DbConn, [], Query),
	mongo_cursor:cursor (DbConn, Query #'op_msg'.collection,
		Query #'op_msg'.batchsize, Query #'op_msg'.slaveok, query_reply (Reply)).
...
query_reply ([Reply]) ->
	case bson:at(errmsg, Reply) of
		null ->
			...
			Docs = bson:at(firstBatch, FirstBatch),
