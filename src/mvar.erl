...
create (Initialize, Finalize) ->
	...
	case gen_server:start_link (?MODULE, {self(), Ref, Initialize, Finalize}, []) of
		...
		ignore -> 
                    receive 
                        {mvar_init_throw, Ref, Thrown} -> throw (Thrown) 
                    end 
        end.
...
init ({Caller, ThrowRef, Initialize, Finalize}) -> 
        try Initialize() of
            A -> 
                %io:format("~p:init ~p Initialize = ~p, Finalize =~p ~n", [?MODULE, ?LINE, Initialize, Finalize]), 
                {ok, {A, Finalize}}
	catch 
            Thrown -> Caller ! {mvar_init_throw, ThrowRef, Thrown}, 
            ignore 
        end.
...
handle_call ({modify, Modify}, _From, {A, X}) -> 
    %io:format("~p:handle_call ~p Modify = ~p, A = ~p, X =~p ~n", [?MODULE, ?LINE, Modify, A, X]), 
        try Modify (A) of 
            {A1, B} -> 
                {reply, {ok, B}, {A1, X}}
	catch Thrown -> 
            {reply, {throw, Thrown}, {A, X}} 
        end;
% Terminate mvar
