...
-export ([cursor/5]). % for mongo_query
...
-spec cursor (mongo_connect:dbconnection(), collection(), batchsize(), boolean(),
		{cursorid(), [bson:document()]}) -> cursor(). % IO
%@doc Create new cursor from result batch
cursor (DbConn, Collection, BatchSize, Slaveok, Batch) ->
	mvar:new ({{DbConn, Collection, BatchSize, Slaveok}, Batch}, fun finalize/1).
...
next (Cursor) ->
	Next = fun ({Env, Batch}) ->
		?DEBUG("~p: ~p , Env = ~p , Batch = ~p~n", [?MODULE, ?LINE, Env, Batch]),
		%%Env = {{octopus,{connection,{"solo",27017},<0.366.0>,false,infinity}},octopus_didi,0}
		%%Batch =
		{Batch1, MDoc} = xnext (Env, Batch),
		{{Env, Batch1}, MDoc} end,
	try mvar:modify (Cursor, Next) of
		{} ->
			close (Cursor), {};
		{Doc} ->
			{Doc}
...
xnext (Env = {DbConn, Coll, BatchSize, Slaveok}, {CursorId, Docs}) -> case Docs of
	[Doc | Docs1] ->
		{{CursorId, Docs1}, {Doc}};
	[] -> case CursorId of
		0 -> {{0, []}, {}};
...
batch_reply ([Reply])  ->
	case bson:at(errmsg, Reply) of
		null ->
			FirstBatch = bson:at(cursor, Reply),
			Docs = bson:at(nextBatch, FirstBatch),
			CursorId = bson:at(id, FirstBatch),
			{CursorId, Docs};
		Error ->
			throw (Error)
	end.
...
finalize ({{DbConn, _, _, _}, {CursorId, _}}) -> case CursorId of
	0 -> ok;
